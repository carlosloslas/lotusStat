...
def check_signal_convergence(df, column, signal_range=(0,1)):
    """
        Calculates signal stats during given interval and during the
    last 50% of the interval. If the stats match, the signal is assumed
    to have converged.
    ---
        From some prelimirarly tests it does not seem like the best way
    to progress. I have not found consistent results. Perhaps should
    look into the number of significant figures it would take the
    rounding of the stats to converge.
    """
    mn_signal, mx_signal = signal_range
...
if __name__ == '__main__':
    ...
    fig1, ax1 = plot_lift_signal(data_df, show_visc=True, plot_stats=True, stats=lift_stats, show_stats=True, figsize=(10,5))
    fig2, ax2 = plot_drag_signal(data_df, plot_stats=True, stats=drag_stats, show_stats=True, figsize=(10,5))
    #plt.show
    plt.close()
    plt.close()

    save_figures_to_pngs([(fig1, 'lift.png'), (fig2, 'drag.png')])
    save_figures_to_pdf([fig1, fig2], 'report.pdf')
